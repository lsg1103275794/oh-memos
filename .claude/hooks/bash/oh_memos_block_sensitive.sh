...
if [ -z "$message" ]; then
    ...
    for pattern in "${high_patterns[@]}"; do
        if [[ "$file_path_lower" == *"$pattern"* ]]; then
            message="⚠️ Warning: Editing sensitive file\n   File: $file_path\n   → Consider: oh-memos_save(..., memory_type=\"CONFIG\") after changes"
            break
        fi
    done
fi
...
if [ -z "$message" ]; then
    ...
    for pattern in "${config_patterns[@]}"; do
        if [[ "$file_path_lower" == *"$pattern"* ]] && [[ "$file_path_lower" != *".claude/settings.json"* ]]; then
            message="⚙️ Config file edit → Remember to save important changes with oh-memos_save(..., memory_type=\"CONFIG\")"
            break
        fi
    done
fi
