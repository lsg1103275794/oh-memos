...
if [[ "$tool_name" == "Edit" || "$tool_name" == "Write" ]]; then
    ...
    for pattern in "${config_patterns[@]}"; do
        if [[ "$file_path_lower" == *"$pattern"* ]]; then
            suggestion="⚙️ Config file modified → Consider: memos_save(..., memory_type=\"CONFIG\")"
            break
        fi
    done
    ...
    if [ -z "$suggestion" ]; then
        ...
        for pattern in "${milestone_files[@]}"; do
            if [[ "$file_path_lower" == *"$pattern" ]]; then
                suggestion="📌 Project file updated → Consider: memos_save(..., memory_type=\"MILESTONE\")"
                break
            fi
        done
    fi
    ...
    if [ -z "$suggestion" ]; then
        if [[ "$file_path_lower" == *"test"* || "$file_path_lower" == *"spec"* ]]; then
            suggestion="🧪 Test file modified → If fixing a bug: memos_save(..., memory_type=\"BUGFIX\")"
        fi
    fi
fi
...
if [[ "$tool_name" == "Bash" ]]; then
    ...
    if echo "$result_lower" | grep -qiE "error:|failed|exception|traceback|fatal:|command not found|permission denied"; then
        suggestion="❌ Command failed → Consider: memos_search(query=\"ERROR_PATTERN <error>\") for solutions"
    fi

    # Detect successful fixes
    if [ -z "$suggestion" ]; then
        if echo "$cmd_lower" | grep -qiE "git commit" && echo "$result_lower" | grep -qiE "fix|bug|resolve"; then
            suggestion="✅ Fix committed → Consider: memos_save(..., memory_type=\"BUGFIX\")"
        fi
    fi
    ...
    if [ -z "$suggestion" ]; then
        if echo "$cmd_lower" | grep -qiE "pytest|npm test|cargo test|go test"; then
            if echo "$result_lower" | grep -qiE "passed|ok|success"; then
                suggestion="✅ Tests passed → If this fixed something: memos_save(..., memory_type=\"BUGFIX\")"
            fi
        fi
    fi
    ...
    if [ -z "$suggestion" ]; then
        if echo "$cmd_lower" | grep -qiE "pip install|npm install|cargo add"; then
            if ! echo "$result_lower" | grep -qiE "error:|failed"; then
                suggestion="📦 Dependencies changed → Consider: memos_save(..., memory_type=\"CONFIG\")"
            fi
        fi
    fi
fi
