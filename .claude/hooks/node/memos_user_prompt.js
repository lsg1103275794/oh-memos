...
function detectIntents(prompt) {
  ...
  if (historyPatterns.some(p => p.test(prompt))) {
    intents.push({
      type: 'history_query',
      suggestion: '→ Consider: memos_search to find related past work'
    });
  }
  ...
  if (errorPatterns.some(p => p.test(prompt))) {
    intents.push({
      type: 'error_report',
      suggestion: '→ Consider: memos_search(query="ERROR_PATTERN ...") for past solutions'
    });
  }
  ...
  if (completionPatterns.some(p => p.test(prompt))) {
    intents.push({
      ...
      suggestion: '→ Consider saving: MILESTONE (big feature) / BUGFIX (fix) / FEATURE (new)'
    });
  }
  ...
  if (relationPatterns.some(p => p.test(prompt))) {
    intents.push({
      ...
      suggestion: '→ Consider: memos_get_graph or memos_trace_path for causal chains'
    });
  }
  ...
  if (statusPatterns.some(p => p.test(prompt))) {
    intents.push({
      type: 'status_query',
      suggestion: '→ Consider: memos_list or memos_get_stats for project overview'
    });
  }
